  ...
  async function updateExchangeRates() {
    try {
      const response = await fetch(
        "https://api.exchangerate-api.com/v4/latest/TTD"
      );
      ...
      document.getElementById(
        "usd-rate"
      ).innerHTML = `<span class="rate-value">${ttdToUsd} TTD</span> = 1 USD`;
      document.getElementById(
        "gbp-rate"
      ).innerHTML = `<span class="rate-value">${ttdToGbp} TTD</span> = 1 GBP`;
      document.getElementById(
        "eur-rate"
      ).innerHTML = `<span class="rate-value">${ttdToEur} TTD</span> = 1 EUR`;
